 ...
 * ------------------------------ */
defined('MAX_PER_SITEMAP') or define('MAX_PER_SITEMAP' , 100 );
	...
	*/
	...
	public function get_archives($post_type = 'post', $type = '') 
	{
		...
		if ( 'monthly' == $type ) {
			...
			$query= apply_filters( 'sitemap_archives_query' , $query );	
			$key = md5($query);
			if ( !isset($this->_cache_archives)){
				$this->_cache_archives = wp_cache_get( 'xmlsf_get_archives' , 'general');
			}
			
			if ( !isset( $this->_cache_archives[ $key ] ) ) {
				$arcresults = $wpdb->get_results($query);
				$this->_cache_archives[ $key ] = $arcresults;
				wp_cache_set( 'xmlsf_get_archives', $this->_cache_archives, 'general' );
			} else {
				$arcresults = $this->_cache_archives[ $key ];
			}
			if ( $arcresults ) {
				foreach ( (array) $arcresults as $arcresult ) {

					if ( MAX_PER_SITEMAP && (int)$arcresult->posts > MAX_PER_SITEMAP ) {
						$pages = ceil( $arcresult->posts / MAX_PER_SITEMAP ) ;
						for ( $i=1; $i<=$pages ;$i++ ){
							if($i==1){
								$return[$arcresult->year.$arcresult->month.'-'.$i] = esc_html( $this->get_index_url( 'posttype', $post_type, $arcresult->year . $arcresult->month ) );	


							}else{
								$return[$arcresult->year.$arcresult->month.'-'.$i] = add_query_arg( 'paged', $i , $this->get_index_url( 'posttype', $post_type, $arcresult->year . $arcresult->month ) ); 
			...
			if ($arcresults) {
				foreach ( (array) $arcresults as $arcresult) {
					...
					if ( MAX_PER_SITEMAP && (int)$arcresult->posts > MAX_PER_SITEMAP ) {
						...
						for ( $i=1; $i<=$pages ;$i++ ){
							if($i == 1){
								$return[$arcresult->year.'-'.$i] = esc_html($this->get_index_url( 'posttype', $post_type, $arcresult->year ) );

							}else{
							$return[$arcresult->year.'-'.$i] = add_query_arg( 'paged', $i , $this->get_index_url( 'posttype', $post_type, $arcresult->year ) );	
	...
	public function modified($sitemap = 'post_type', $term = '') 
	{
		if ('post_type' == $sitemap) :
			...
			if ( empty($this->postmodified[$post->ID]) ) {
				$postmodified = get_post_modified_time( 'Y-m-d H:i:s', true, $post );
	...
	*/
	...
	public function filter_request( $request ) 
	{
		if ( isset($request['feed']) && strpos($request['feed'],'sitemap') == 0 ) {
			...
			do_action('sitemap_request' , $request );
			...
			if ( strpos($request['feed'],'sitemap-posttype') == 0 ) {
				foreach ( $this->get_post_types() as $post_type ) {
					if ( $request['feed'] == 'sitemap-posttype-'.$post_type['name'] ) {
						...
						// Paginated sitemap
						if( MAX_PER_SITEMAP && isset($request['m']) ){
							$request['posts_per_rss'] = MAX_PER_SITEMAP ;
						} else {
