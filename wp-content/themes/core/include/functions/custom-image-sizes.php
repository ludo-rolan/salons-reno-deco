...
class RW_Filosofo_Custom_Image_Sizes {
	...
	private function _generate_attachment($attachment_id = 0, $width = 0, $height = 0, $crop = true)
	{
		...
		// We dont want to run getimagesize multiple times on the same attachement
		global $_valid_images;
		if ( !isset( $_valid_images[$attachment_id] ) ){
			// check the image first
			$valid = getimagesize($original_path);
			if ( !$valid ) 
				$_valid_images[$attachment_id] =false;
			else $_valid_images[$attachment_id] =true;

		} else {
			$valid = $_valid_images[$attachment_id];
		}
...
function rw_initialize_custom_image_sizes()
{
	new RW_Filosofo_Custom_Image_Sizes;
}
rw_initialize_custom_image_sizes();
