...
function get_project_nl_id(){
    return (PROJECT_NAME == "INFINIDECODEAUVILLE")?"x_ImportInfiniDecoDeauville":"x_ImportsalonArtDeco";
}
...
class GAIACRM
{
     ...
     * mettre x_Import_VisiteursalonAD true  
     */
    ...
    public function synchronyzeDbToGaia($email)
    {
        ...
        if (isset($user)){
            $project_id = get_project_nl_id();
            if (!$user->$project_id) $this->subscribeToNL($email,$user->Id);
        }
        else{
