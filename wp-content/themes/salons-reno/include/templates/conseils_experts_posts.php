<?php
	...
	if( !$categorie_link ){
		$cat_conseils_experts = RW_Category::rw_get_category_by_slug('conseils-experts');
		$categorie_link = get_category_link($cat_conseils_experts); 
	}

?>
<div class="bloc_rubrique_head">
	<h2 class="page-title"><?php _e('conseils d’experts'); ?></h2>
	<?php if( !$hide_read_more_link ){ ?>
		<a class="read_more" href="<?php echo $categorie_link; ?>"><?php _e('voir tous les conseils') ?></a>
	<?php } ?>
</div>
<div class="row">
	<?php
	if( apply_filters('conseils_experts_by_cat', false, $conseils_experts_cat) ){
		$posts = get_data_from_cache('conseils_experts_posts', 'home_rubrique', 60*60*24, function(){
			...
			if( defined('_LOCKING_ON_') && _LOCKING_ON_&&  $args_lock = get_locking_config('sous_category', 'conseils-experts'.$current_cat->slug)){
